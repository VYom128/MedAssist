...
| `KIOSK_KEY` | (≥ 24 random chars) | Queue board key (`/queue-board?key=…` and the kiosk socket); unset = board off (Phase 4, D87) |
| `JOBS_ENABLED` | false | Run the reminder and no-show jobs in this API process (one instance only; Phase 4, D90) |

**Client**: `VITE_API_URL`, `VITE_SOCKET_URL`, `VITE_APP_NAME`, `VITE_REALTIME` (`off` disables Socket.IO; the tests use it).
...
| D41 | 2026-09-23 | Doctors are identified by their **User id** everywhere: `/doctors/:id`, `schedule.doctor`, `leave.doctor` and (from Phase 4) `appointment.doctor`. The DoctorProfile id is internal. | One id per person; auth already knows it. |
| D42 | 2026-09-23 | Doctor accounts are created only by `POST /doctors` (User + DoctorProfile in one transaction, welcome email after commit). `POST /users` rejects `doctor` with a message pointing to `/doctors` (supersedes the doctor part of D34). A doctor's name, email and phone are edited through `PATCH /users/:id`; account status through `/users/:id/activate|deactivate`. | No doctor without a profile. |
| D43 | 2026-09-23 | `PUT /doctors/:id/schedule` and `POST /doctors/:id/leaves` return `affectedAppointments: []` with `TODO(Phase 4)`. Slots, `/slots`, `/availability` and finding affected appointments are Phase 4. | Appointments do not exist yet. |
| D44 | 2026-09-23 | Clinic `logoUrl` is an http(s) URL until file uploads arrive in Phase 6. | Uploads are Phase 6. |
| D45 | 2026-09-23 | Clinic settings are one document (`key: 'clinic'`, created with the §6.5 defaults on first read) read through `getSettings()`, cached in memory, replaced after every update, with a 60 s TTL so other API instances catch up (§17). Reading never writes. `PATCH /settings` is a deep partial update; unknown keys → 400; audited with before/after of the changed dotted paths. Currency stays editable (3-letter code); timezone must be an IANA Area/Location name (aliases such as Asia/Kolkata accepted, abbreviations such as IST rejected). No `workingHours` field (the public subset is the list given in the step 1 prompt). | Performance and a single source of truth. |
| D46 | 2026-09-23 | The API accepts and returns money only as integer paise (Zod `paise`, Mongoose integer validators). The UI lets people type rupees (`MoneyInput`, half-up rounding on the digits, no float maths). Tax rates are basis points in the API and a percentage in the UI. | §3.7; no float drift. |
| D47 | 2026-09-23 | Weekly schedules are versioned: a version is 7 documents (one per weekday; `sessions: []` = day off) sharing `effectiveFrom`. `effectiveFrom`/`effectiveTo` are clinic calendar dates stored at UTC midnight; `effectiveTo` is inclusive, null = open. `PUT` (in a transaction) deletes versions starting on or after the new date, closes the open one the day before, and inserts the new one; `effectiveFrom` must be today or later (clinic time; the seed may backdate). Sessions: `HH:mm` clinic time, 5-minute steps, start < end, no overlaps (touching allowed), ≤ 6 a day; `maxWalkIns` defaults to `settings.appointment.walkInOverbookPerSession`. Sessions on non-working days are saved with a warning. `getScheduleForDate(doctorId, 'YYYY-MM-DD')` is the Phase 4 entry point. | Past bookings must keep the hours that applied then. |
| D48 | 2026-09-23 | Phase 2 stores and validates the lab catalogue only; reference-range helpers (choosing a range, computing flags) are Phase 6. Rules: ≥ 1 parameter; unique keys; `option` needs ≥ 2 distinct options; ranges only on `number`; low < high, criticalLow ≤ low, high ≤ criticalHigh, criticalLow < criticalHigh; ages whole years, both ends inclusive; ranges of the same gender must not overlap in age; an `any` range may coexist with male/female ones (Phase 6 prefers the specific gender). `POST /lab-tests/:id/activate` added (§7.14 lists only deactivate). | §6.19; flags are Phase 6 work. |
| D49 | 2026-09-23 | Leave is stored as `[startAt, endAt)` in UTC (end exclusive; a full day ends at the next clinic midnight) with both `type` and `reason`. `POST` takes `{ startAt, endAt }` (ISO instants) or `{ date, endDate?, fullDay: true }` (server converts with the clinic timezone). Rules: end after start, ≤ 90 days, not in the past (may start earlier today), no overlap with the doctor's non-cancelled leave (409). The overlap check and insert run in a transaction that bumps `DoctorProfile.lockVersion`, so concurrent requests cannot both succeed. Cancel only before `endAt`. `GET` defaults `from` to today. | §4.13, §6.10. |
| D50 | 2026-09-23 | Public reads (`GET /settings/public`, `/departments`, `/departments/:id`, `/services`, `/services/:id`, `/doctors`, `/doctors/:id`) use `optionalAuthenticate`: no token = public view; a bad token = 401; an admin gets the admin view and may pass `includeInactive`. Inactive records are 404 to non-admins. `GET /lab-tests` needs a login; patients see code, name, category, sample type, price and preparation. | §2.4, §7.5–7.6, §7.14. |
| D51 | 2026-09-23 | Accepted step deviations: (a) settings cache TTL 60 s; (b) admins get the admin view in `GET /doctors` too; (c) leave `endDate` for several whole days; (d) `/lab-tests/:id/activate`; (e) no settings `workingHours`; (f) the seed may backdate the first schedule version 90 days. | Approved by the project owner. |
| D52 | 2026-09-23 | Department deactivation is blocked with `409 CONFLICT` and `details.activeDoctors` while active doctors belong to it. The admin department view includes `activeDoctors`. Services: department optional (clinic-wide), must be active; admins may filter by `isActive`; the public list hides services of inactive departments. | §7.5. |
| D53 | 2026-09-23 | Doctor self-service: a doctor may `PATCH` only their own `bio` and `languages` (other fields → 403 with `details.fields`) and manage their own schedule and leave. Acting on another doctor → 403 (doctors are public, so 404 hides nothing), audited as `access.denied` with reason `not_own_doctor`. Rules live in `policies/doctorAccess.ts`. The doctor's own profile page shows no admin-only fields (email, phone, registration no., room, slot length, status). | §2.4, §7.6. |
| D54 | 2026-09-23 | Audit actions added beyond §10.4: `settings.update` (dotted-path before/after), `department.create|update|activate|deactivate`, `service.create|update|activate|deactivate`, `doctor.create`, `doctor.update`, `doctor.leave_cancel`, `lab_test.create|update|activate|deactivate`. The RBAC matrix test asserts that every allowed write writes an audit entry. The seed calls the services as the demo admin with `request: { method: 'SEED', path: 'npm run seed' }`. | §10.4. |
| D55 | 2026-09-23 | The audit writer reads the chain head from the database for every write (one indexed query) and retries up to 3 times when another writer takes the same `seq`. Replaces D13's cached head: a seed `--reset` while the API was running broke the chain, and a second writer could make an entry fail. Still serialised per process. | Found by the Phase 2 smoke test. |
| D56 | 2026-09-23 | Seed v1 (§15.3): settings, 5 departments, 12 services, 8 doctors (dr.mehta GEN, dr.iyer PED + 6 generated by `@faker-js/faker` `en_IN`, fixed seed) with profiles, schedules and 2 upcoming leaves, 2 receptionists, 2 lab techs, 15 lab tests. Upserts by email/code through the services and their Zod schemas; no welcome emails; repairs Phase 1 doctor accounts without a profile; `--reset` also clears the Phase 2 collections and counters. Local MongoDB must be a replica set from Phase 2. Supersedes D22/D36 on scope. | Realistic demo data through the real rules. |
...
| D79 | 2026-09-24 | **Booking lock.** Booking, rescheduling, walk-ins and undo-no-show run in `withTransaction` and first increment `bookingVersion` on the DoctorProfile and the Patient; two overlapping transactions write the same document, MongoDB aborts one, the driver retries it and the retry sees the first booking and fails its checks. The partial unique index `{ doctor, startAt }` (`isSlotActive: true`) is the last guard; a duplicate key or an unretried write conflict → 409 `SLOT_UNAVAILABLE`. Leave and schedule writes bump `bookingVersion` too (as well as `lockVersion`), so they serialise with bookings. | Two people must never book the same slot; the index alone misses overlapping multi-slot services. |
| D80 | 2026-09-24 | **Clash rules (§8.2).** Doctor clash = any `isSlotActive` appointment overlapping `[startAt, endAt)`; patient clash and "same doctor same day" count *open* appointments only (scheduled, checked in, in consultation – not completed). A service longer than a slot needs consecutive free time. The patient booking limit counts upcoming scheduled appointments (patients only; staff may exceed it). The 15-minute lead time applies to everyone; clinic `workingDays` apply on top of doctor schedules; `serviceId` is required. §4.5's "daily limit" is read as the same-doctor-same-day rule. | Precise, testable rules. |
| D81 | 2026-09-24 | **State machine.** Transition tables in `STATE_MACHINES` (config/constants.ts); `assertTransition()` lives in `utils/stateMachine.ts` (it throws ApiError, which imports the constants). Every status change goes through `applyTransition()`: `assertTransition`, one conditional update `{ _id, status: from }` (concurrent actions: the loser gets 409), `statusHistory` entry, `isSlotActive` set with the status (the model recomputes it on save and rejects update queries that change status without it). Reschedule only from scheduled; undo-no-show checks for `no_show` explicitly (the table also allows scheduled → scheduled). New audit actions beyond §10.4: `appointment.update`, `appointment.undo_no_show`, `appointment.priority_change`. | §5.1; one path for every status change. |
| D82 | 2026-09-24 | New error codes `SELF_BOOKING_DISABLED` (403), `OUTSIDE_BOOKING_WINDOW` (422), `OVERBOOK_LIMIT_REACHED` (422) (§16). | Clear client messages. |
| D83 | 2026-09-24 | `POST /appointments/:id/start` only changes the status (`TODO(Phase 5)`: encounter draft); cancelling does not touch billing (`TODO(Phase 7)`). A doctor has at most one consultation in progress per clinic day (start and call-next lock the doctor; 409 `CONFLICT`); one left open on an earlier day does not block. | Scope; a forgotten consultation must not lock a doctor out. |
| D84 | 2026-09-24 | **Walk-ins** take the next free slot of the session running now (no lead time); if none, an overbook starting now (`isSlotActive: false`, `isOverbook: true`) up to `settings.appointment.walkInOverbookPerSession` per session (the Phase 2 per-session `maxWalkIns` is not used) → 422 `OVERBOOK_LIMIT_REACHED`. No session running, leave, or not accepting → 409 `DOCTOR_UNAVAILABLE`. Created checked in with a token (`type`/`source` walk_in). | Project owner's decision. |
| D85 | 2026-09-24 | **No-show / priority.** Reception may mark a no-show only once the appointment has started; undo on the same clinic day only, if the slot and the patient are still free; it sets `noShowUndoneAt` (new field) so the job leaves it alone. `POST /queue/:id/priority` only while checked in, reason required, kept in `priorityHistory` (new field); `PATCH /appointments/:id` changes priority/reason before check-in. | Avoids the job re-marking an undone no-show; priority reasons kept. |
| D86 | 2026-09-24 | **Tokens** come from the counter `token:<doctorId>:<YYYY-MM-DD>` inside the check-in/walk-in transaction (parallel check-ins get distinct tokens; aborted ones use none). Audit metadata calls it `queueNumber`: the audit redactor masks every key containing "token". Appointment numbers `APT-<clinic year>-NNNNNN` from `appointment:<year>`. | §8.4, §8.10. |
| D87 | 2026-09-24 | **Queue and board.** Order: priority → scheduled time (walk-ins: check-in time) → check-in time → token. Estimated wait = patients ahead (the one with the doctor counts as one) × the doctor's average consultation over 30 days, else the slot length. The board lists active doctors with appointments today: name, room, token now, next five tokens – no patient data and no ids. `KIOSK_KEY` (≥ 24 chars, constant-time compare; unset → 404, wrong → 401), rate-limited 60/min/IP. Open decision 5 is settled as a static key in the URL, not a signed one. | §4.6 step 6. |
| D88 | 2026-09-24 | **Socket.IO.** Handshake with the access token through `resolveAccessToken()` (the same checks as `authenticate`), or `{ kioskKey }`. Rooms `user:<id>`, `queue:<doctorId>:<date>` (joined with `queue:subscribe`: staff any doctor, a doctor their own, a patient the doctor they see that day; the session is re-checked on every subscribe), `board`. Events `queue.updated { doctorId, date }` and `appointment.changed { appointmentId }` – ids only, emitted after the commit. Logout does not disconnect an open socket (events carry no data). Client: one socket per login, loaded lazily, reconnected with the new token after a refresh; `VITE_REALTIME=off` in tests. | §7.9; no PHI over sockets. |
| D89 | 2026-09-24 | **Notifications** through `notification.service.notify()` – email only until Phase 10. Appointment emails carry the number, date/time (clinic time) and clinic only – no doctor, department or reason; the doctor is emailed for same-day changes; receptionists are emailed when leave affects bookings (§11 says in-app: email until Phase 10). Reminders go to appointments `reminderHoursBefore` ± 15 min away, claimed by setting `reminderSentAt` first (never twice); a reschedule clears it; bookings made less than ~24 h ahead get only their confirmation. | §10.3 (no PHI in email). |
| D90 | 2026-09-24 | **Jobs**: `runReminderJob(now)` and `runNoShowJob(now)` exported for tests; node-cron every 15 min in the clinic timezone read at startup (a timezone change needs a restart), only when `JOBS_ENABLED=true`, on one API instance; each logs a one-line summary and never throws. | §8.11. |
| D91 | 2026-09-24 | **Access.** Reception sees and acts on all appointments. Admins see all (list, detail) but the UI offers no actions (the API still lets them book/reschedule/cancel per §7.8, which conflicts with §2.4 "R"). Doctors see their own, with the minimal patient view (name, MRN, age, sex, stated reason); their UI offers start and complete only (the API also allows cancelling their own). Patients see their own; anything else → 404 (`access.denied`). Detail pages at `/reception|admin|doctor/appointments/:id`. | Project owner's decisions. |
| D92 | 2026-09-24 | **Patients online** may cancel or reschedule until `minCancelHours` before the start (422 `CANCELLATION_WINDOW_PASSED`; checked in → "speak to reception"); rescheduling also needs self-booking on. `GET /settings/public` now includes `appointment.minCancelHours`. Staff reasons ≥ 3 chars; patient reasons optional; audit entries record `reasonGiven`, not the text (PATCH reason values `[REDACTED]`). | §8.3; no free text in the audit log. |
| D93 | 2026-09-24 | **Leave/schedule impact (§4.13):** `affectedAppointments` = scheduled or checked-in appointments overlapping the leave, or (schedule) upcoming ones outside the new sessions from `max(effectiveFrom, now)`; leave over an appointment in consultation or completed → 409. | Fills D43. |
